...
乐观锁和悲观锁的介绍以及乐观锁常见实现方式可以阅读笔者写的这篇文章：[乐观锁和悲观锁详解](./01_乐观锁和悲观锁详解.md)。
 ...
 * 以原子方式更新对象字段的值。
 *
 * @param o         要操作的对象【CAS 修改的是：对象中的某个字段，而不是单独修改一个变量】
...
>                     UnsafeCasObjectDemo.class.getDeclaredField("name")
>             );
> 
>         } catch (Exception e) {
>             e.printStackTrace();
...
> import java.lang.reflect.Field;
> 
> public class UnsafeCasLongDemo {
> 
>     private volatile long count = 100L;
> 
>     private static Unsafe unsafe;
...
>                     UnsafeCasLongDemo.class.getDeclaredField("count")
>             );
> 
>         } catch (Exception e) {
>             e.printStackTrace();
...
>     public static void main(String[] args) {
> 
>         UnsafeCasLongDemo demo = new UnsafeCasLongDemo();
> 
...
>                     UnsafeCasIntDemo.class.getDeclaredField("value")
>             );
> 
>         } catch (Exception e) {
>             e.printStackTrace();
...
>     public static void main(String[] args) {
> 
>         UnsafeCasIntDemo demo = new UnsafeCasIntDemo();
> 
>         System.out.println("修改前：" + demo.value);
> 
>         // CAS:
>         // 期望值 10
>         // 更新值 20
...
>         System.out.println("修改后：" + demo.value);
>     }
> }
> ```
>
...
`java.util.concurrent.atomic` 包提供了一些用于**原子操作类**，这是实际开发中常常使用的方式，而不是强行开启 `Unsafe` 类。

![](./../assets/JUC%E5%8E%9F%E5%AD%90%E7%B1%BB%E6%A6%82%E8%A7%88-20230814005211968.png)

关于这些 Atomic 原子类的介绍和使用，可以阅读这篇文章：[Atomic 原子类总结](./08_Atomic原子类总结.md)。
...
不过，从 JDK 1.5 开始，Java 提供了`AtomicReference`类，这使得我们能够保证引用对象之间的原子性。通过将多个变量封装在一个对象中，我们可以使用`AtomicReference`来执行 CAS 操作。
