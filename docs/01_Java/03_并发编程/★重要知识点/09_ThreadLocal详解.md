...
> 说明：使用**开放寻址法**的查找逻辑：null意味着“后面不可能再有冲突元素”，因此对于key为null的Entry不能直接删除该Entry，会导致后续查找链断裂。因此需要对 key 为 null 的 Entry 进行清理并 rehash 后面的元素。

![](./../assets/12-CF9wKPyx.png)
...
private void set(ThreadLocal<?> key, Object value) {
    ...
    for (Entry e = tab[i];
        ...
        // 情况2：key为null
        if (k == null) {
            // 清理脏Entry（key为null，但Entry本身和value都还在）并完成赋值
...
`slotToExpunge`表示开始探测式清理过期数据的开始下标，默认从当前的`staleSlot`开始。以当前的`staleSlot`开始，**向前**迭代查找，找到没有过期的数据，`for`循环一直碰到`Entry`为`null`才会结束。如果向前找到了过期数据，更新探测清理过期数据的开始下标为 i，即`slotToExpunge=i`
...
## 8、`ThreadLocalMap`过期 key 的探测式清理流程

上面我们有提及`ThreadLocalMap`的两种过期`key`数据清理方式：**探测式清理**和**启发式清理**。

我们先讲下 **探测式清理** ，也就是`expungeStaleEntry`方法：

遍历散列数组，从开始位置向后探测，清理过期数据，将过期数据的`Entry`设置为`null`，沿途中碰到未过期的数据则将此数据`rehash`后重新在`table`数组中定位，如果定位的位置已经有了数据，则会将未过期的数据放到最靠近此位置的`Entry=null`的桶中，使`rehash`后的`Entry`数据距离正确的桶的位置更近一些。操作逻辑如下：

![](./../assets/18-ZS4gusY8.png)
...
这里首先是会进行探测式清理工作，从`table`的起始位置往后清理，上面有分析清理的详细流程。清理完成之后，`table`中可能有一些`key`为`null`的`Entry`数据被清理掉，所以此时再通过判断`size >= threshold - threshold / 4` 也就是`size >= threshold * 3/4` 来决定是否扩容。

我们还记得上面进行`rehash()`的阈值是`size >= threshold`，所以当面试官套路我们`ThreadLocalMap`扩容机制的时候 我们一定要说清楚这两个步骤：

![](./../assets/24-9PXxomoR.png)
...
![](./../assets/26-DO12ex-R.png)

**第二种情况：** `slot`位置中的`Entry.key`和要查找的`key`不一致：

![](./../assets/27-DGESCpsE.png)
...
## 11、`ThreadLocalMap` 过期 key 的启发式清理流程
...
通常 `set()`  插入新Entry后，会调用：`cleanSomeSlots(i, size)` ，它的目的不是彻底清理整个Map，而是：从当前位置开始，顺手探测少量slot，看看有没有：`stale entry(key=null)` 的脏Entry，有的话立即触发深度清理（`expungeStaleEntry(i) `，探测式清理）。

每循环一次：**n右移1位**，即 `n /= 2` ，因此最终循环次数约等于 **log2(n)**。

### 11.1、完整流程

假设：table长度 = 16，现在：`cleanSomeSlots(5, 16)`
...
- **第三步：判断是否 stale**
  ...
  判断：`e != null && e.get() == null` 

  即：Entry存在，但key被GC

- **如果不是 stale**

  例如：`6 -> 正常Entry`，则什么都不做，继续。
...
- **再往后扫描**
  ...
  因此 最终只扫 **log2(16)=4** 次左右

- **如果发现 stale（关键）**

  例如：`8 -> stale` ，即 `key = null` ，那么：
  ...
  - 第二步（重点）
    ...
    > 因为：既然已经发现脏区域，说明附近可能还有更多 stale，因此扩大扫描范围
  ...
  - 第三步（最重要）
    ...
    > ⚠ 注意：启发式清理 自己**并不负责真正彻底清理**，它只是“侦察兵”，发现问题后交给：` expungeStaleEntry()`，做：
    >
    > - 连续清理
    > - rehash
    > - 修复探测链
...
- **清理后继续扫描**
  ...
  `expungeStaleEntry()`  返回新的有效位置，`cleanSomeSlots` 继续从这里往后探测

### 11.2、为什么叫“启发式”

因为它 **不保证全扫** ，只是从当前位置附近随机/概率性扫一点，不是整个数组全面大扫除。
...
因为 **stale entry 往往是局部聚集**的。

ThreadLocalMap 使用线性探测，因此冲突元素会聚集，而stale通常也会聚集，所以一旦发现一个 stale，附近很可能还有更多，因此 `n = len;`扩大扫描。

### 11.4、cleanSomeSlots 本质是什么
...
| 默认少量扫描        | log2(n)                   |
| 发现stale后扩大范围 | n=len                     |
| 不自己深度清理      | 调expunge，本身不负责清理 |
| 目标                | 避免脏Entry长期积累       |

它只是“**发现问题**”，真正删除 stale、rehash、修复探测链，都是：`expungeStaleEntry()`
...
实现原理是子线程是在父线程中通过调用`new Thread()`方法来创建子线程，`Thread#init`方法在`Thread`的构造方法中被调用。在`init`方法中拷贝父线程数据到子线程中：
