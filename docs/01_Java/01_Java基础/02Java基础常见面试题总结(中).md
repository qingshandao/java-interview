...
两种拷贝，都需要实现 `Cloneable` 接口，并重写 `clone()`方法：

- 重写的  `clone()` 方法是 `Object` 中的 `clone()` 方法；

-  `Object` 中的 `clone()` 方法是一个 `protected` 方法，且其默认实现会：

  - 复制对象的字段（二进制级别的浅拷贝）；
  - 在对象没有实现 `Cloneable` 接口时，会抛出 `CloneNotSupportedException` 异常；

- `Cloneable` 接口的作用：

  - Cloneable 是 Java 提供的一个标记接口（marker interface），它不包含任何方法，其唯一作用是：告诉 Object 类中的 clone() 方法：当前这个类是允许被克隆的。
...
/**
 * 用于比较 2 个对象的内存地址是否相等，String 类对该方法进行了重写以用于比较字符串的值是否相等，Integer类也重写用以比较值相等。
 */
...
举个例子（这里只是为了举例。实际上，按照下面这种写法的话，像 IDEA 这种比较智能的 IDE 都会提示你将 `==` 换成 `equals()` ）：
...
```java
String pool = "ab";
String heap = new String(pool);

System.out.println(pool == heap);            // false：不同对象
System.out.println(pool.equals(heap));       // true：内容相同
System.out.println(pool.intern() == heap.intern()); // true：都指向常量池中 "ab"

```

> 常量池中，已有 ”ab“ ，因此 intern() 返回 `pool` 和 `heap` 对象的字面值相同的在常量池中的 ”ab“ 对象，因此最后判断结果为 true


```java
String str1 = new String("a") + new String("b"); // 在堆中创建了 "ab"
String str2 = str1.intern(); // 尝试把堆中的 "ab" 放入常量池

String str3 = "ab"; // 常量池中的 "ab"

System.out.println(str1 == str2); // true（重点）
...
> 步骤解析：
>
> - new String("a") + new String("b") 创建了一个堆上的 "ab" 字符串（JVM 优化前不放入常量池）。
>
> - str1.intern()：
>   - JVM 检查常量池中是否已有 "ab"。
>   - 没有，于是将 str1（堆对象）的**引用**【 ⚠堆中 "ab" 对象的引用地址 加入常量池，而不是实际的”ab“对象】加入常量池，并返回它。
>
> - str2 == str1 成立（引用同一个对象）。
>
> - str3 = "ab"：发现常量池中已经有 "ab"，直接使用，和 str2 同引用。
...
public class Test {
    public static void main(String[] args) {
        ...
        System.out.println(s1 == s2); // true（都是堆中的"abc"地址）, 是否为 true 取决于 s3 是否先出现
        System.out.println(s2 == s3); // 一定为 true
    }
}
...
public class Test {
    public static void main(String[] args) {
        String s0 = "abc"; // 触发 ldc，"abc" 加入运行时常量池
        String s1 = new String("abc");
        String s2 = s1.intern();
        String s3 = "abc";

        System.out.println(s1 == s2); // false（s1是堆中地址，s2是常量池中地址）
    }
}
...
| 问题                                      | 回答                                            |
| ----------------------------------------- | ----------------------------------------------- |
| 编译阶段 "abc" 就放入常量池了吗？         | 放入的是 class 文件的常量池表，不是运行时常量池 |
| "abc" 会在何时进入运行时常量池？          | 在第一次使用 "abc"（触发 ldc）时                |
| intern() 是否可能将堆对象引用放入常量池？ | 可能，只要 "abc" 尚未进入运行时常量池（JDK 7+） |
| 所以 s1.intern() == s1 是否可能为 true？  | 是的，在 "abc" 尚未 ldc 之前可以发生            |
...
![](./assets/java-hashcode-method.png)
...
在前面也提到了添加元素进`HashSet`的过程，如果 `HashSet` 在对比的时候，同样的 `hashCode` 有多个对象，它会继续使用 `equals()` 来判断是否真的相同。也就是说 `hashCode` 帮助我们大大缩小了查找成本。
...
- **性能**
  ...
  - `StringBuffer` 每次都会对 `StringBuffer` 对象本身进行操作，而不是生成新的对象并改变对象引用。相同情况下，使用 `StringBuilder` 相比使用 `StringBuffer` ， `StringBuilder` 仅能获得 10%~15% 左右的性能提升，但却要冒多线程不安全的风险。
...
## 2、String 为什么是不可变的（为什么String变量值是常量）?
...
>
>    > 被 `final` 修饰的引用类型，可以修改数组的内容，final 只是不能修改“引用”，不是不能修改“内容”！
>    >
>    > ```java
...
>    > arr[0] = 'z';   // ✅ 合法，修改了内容
>    > 
>    > arr = new char[]{'x', 'y', 'z'};  // ❌ 编译错误，不能修改引用
>    > 
>    > ```
>
...
![](./assets/image-20220422161637929.png)
...
![](./assets/image-20220422161320823.png)
...
![](./assets/image-20220422162327415.png)
...
更多关于字符串常量池的介绍，可以看一下 [Java 内存区域详解](https://javaguide.cn/java/jvm/memory-area.html) 这篇文章，上文也有提到字符串常量池的内容：[2、⚠== 和 equals() 的区别](##2、⚠== 和 equals() 的区别)
...
- **从常量池加载字符串**：`ldc` 首先检查字符串常量池中是否已经有内容相同的字符串对象。
...
- **常量池中已有相同内容的字符串对象**：如果字符串常量池中已经有一个与调用 `intern()` 方法的字符串 **内容相同（字面值）** 的 `String` 对象，`intern()` 方法会直接返回常量池中该对象的引用。

- **常量池中没有相同内容的字符串对象**：如果字符串常量池中还没有一个与调用 `intern()` 方法的字符串**内容相同（字面值）**的对象，`intern()` 方法会将当前字符串对象的 **引用** 添加到字符串常量池中，并返回该引用。

**【总结】：**

- `intern()` 方法的主要作用是确保字符串引用在常量池中的**唯一性**。
...
其它具体的案例，参考上文：[2、⚠== 和 equals() 的区别](##2、⚠== 和 equals() 的区别)

## 8、String 类型的变量和常量做“+”运算时发生了什么？

### （1）先来看字符串不加 `final` 关键字拼接的情况（JDK1.8）：
...
![](./assets/image-20210817142715396.png)

常量折叠会把常量表达式的值求出来作为常量嵌在最终生成的代码中，这是 Javac 编译器会对源代码做的极少量优化措施之一(代码优化几乎都在即时编译器中进行)。

对于 `String str3 = "str" + "ing";` 编译器会给你优化成 `String str3 = "string";` 。

并不是所有的常量都会进行折叠（直接用结果替换），只有编译器在程序编译期就可以确定值的常量才可以：

- 基本数据类型( `byte`、`boolean`、`short`、`char`、`int`、`float`、`long`、`double`)
- 字符串常量。
- `final` 修饰的 基本数据类型 和 字符串变量
...
### （2） 字符串使用 `final` 关键字声明之后，可以让编译器当做常量来处理。
...
String c = "str" + "ing";// 常量池中的对象
String d = str1 + str2; // 在堆上,StringBuilder 创建的新的对象
...
- 对c来说，编译器在编译阶段就可以确定 c 的结果是 "string"，c 引用的是 常量池中的对象；
- 对d来说，虽然 str1 是 final，值是字面量 "str"，但 str2 是调用 getStr() 方法的返回值，即使你用 `final` 修饰它，编译器也无法在编译期确定其值。

因此：

- 编译器不能进行常量折叠；

- 所以 str1 + str2 会在运行时使用 StringBuilder 进行拼接；

- 最终 d 是一个新的堆对象，而不是常量池中的 "string"。
