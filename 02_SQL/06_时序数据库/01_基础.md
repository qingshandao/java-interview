...
> ⚠注：此版本的 Web UI 未提供后续查看token的功能，因此需此时就保存，token 涉及其它的配置，例如Java连接、Grafana连接、 CLI 工具配置登录等。
>
> ​	否则只能创建一个新的token，并更新相关配置。
...
进入 `influx.exe` 客户端所在目录，命令行中输入如下命令（引号内替换为自己的token）后，在当前目录生成一个 `.json` 文件。
...
以**管理员身份**打开 `nssm.exe` 所在目录的命令行，执行服务注册命令 `nssm install [服务名] [程序路径]`
