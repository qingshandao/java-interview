...
在该目录下打开命令窗口，输入`influxd.exe`即可启动 ：
...
创建完成以后，进入到主页，可以看到它支持的客户端，支持的方案有很多，以及也可以支持从消息队列、系统日志、其他数据库等地方进行导入或写入数据
...
修改存储路径后，重启 InfluxDB，该配置文件会被自动加载和执行。

由于修改了配置文件，InfluxDB会重新生成上述的存储文件，因此需要再次登录和配置用户、数据表等内容。
