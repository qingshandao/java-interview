...
> `ConcurrentHashMap` 通过 **分段锁（JDK 1.7）或 CAS + synchronized + volatile + 自旋机制（JDK 1.8+）**，避免了全表锁，实现了高效且线程安全的并发读写。

## 1、JDK 1.7：**Segment 分段锁机制**

### 🧠 实现思路：

1. 整个 Map 被分为若干个 Segment（默认 16 个）；
2. 每个 Segment 相当于一个独立的小 HashMap，内部加自己的 `ReentrantLock`；
3. 多线程操作不同 Segment 不会互相阻塞。
...
* 支持**高并发写入**，多个线程可以并行写不同的 Segment；
* 比 `Hashtable` 效率高（`Hashtable` 是全表加锁）。

## 2、JDK 1.8 之后：CAS + synchronized 混合机制

### 🧠 实现思路：

1. 不再使用 Segment，直接使用 **Node 数组 + 链表 / 红黑树**；
2. **使用 CAS 操作 + synchronized 控制粒度更小的加锁**；
3. 读操作基本无锁，写操作只对某个桶加锁。
...
* JDK1.7：会计算 hash 落在哪个 Segment，然后只锁这个 Segment；
* JDK1.8：会先计算桶的位置，如果桶为空，用 CAS 无锁插入；如果有冲突，用 synchronized 锁住该桶；
* 无论哪种方式，都**避免了线程间的数据竞争**。
