...
`ArrayBlockingQueue` 有2组常见的生产-消费方法
...
了解了 `put`、`take` 这两个会阻塞的存和取方法之后，我们再来看看阻塞队列中**非阻塞**的入队和出队方法 `offer` 和 `poll`。
...
<img src="../assets/arrayblockingqueue-class-diagram.png" style="zoom:67%;" />
...
2. 如果 `offer` 失败，即代表当前元素入队失败直接抛异常。而 `AbstractQueue` 中并没有对 `Queue` 的 `offer` 的实现，很明显这样做的目的是定义好了 `add` 的核心逻辑，将 `offer` 的细节交由其子类即我们的 `ArrayBlockingQueue` 实现。
...
另外两个构造方法都是基于上述的构造方法，默认情况下，我们会使用下面这个构造方法，该构造方法就意味着 `ArrayBlockingQueue` 用的是**非公平锁**，即各个生产者或者消费者线程收到通知后，**对于锁的争抢是随机的**。
...
<img src="../assets/ArrayBlockingQueue-notEmpty-take.png" style="zoom:67%;" />
...
<img src="../assets/ArrayBlockingQueue-notFull-put.png" style="zoom:67%;" />
...
public void put(E e) throws InterruptedException {
    ...
    try {
       //如果count等于数组长度则说明队列已满，当前线程将被挂起放到AQS队列中，等待队列非满时插入（非满条件）。
...
private E dequeue() {
  ...
  count--;
  if (itrs != null)
      // 通知所有还活着的迭代器，队列中有一个元素被删除了，请自己同步状态。而不是像普通集合那样抛出并发修改异常
...
<img src="../assets/ArrayBlockingQueue-notEmpty-notFull.png" style="zoom:67%;" />
...
* `offer(E e)`：将元素插入队列尾部。如果队列已满，则该方法会直接返回 false，不会因队满而等待并阻塞线程。
* `poll()`：获取并移除队列头部的元素，如果队列为空，则该方法会直接返回 null，不会因队空而等待并阻塞线程。
...
# 三、ArrayBlockingQueue 获取和新增元素的方法对比
