...
* **底层数据结构：**
	-  JDK1.7 的 `ConcurrentHashMap` 底层采用 **分段的数组+链表** 实现，JDK1.8 中的 `ConcurrentHashMap` 的底层数据结构，与 JDK1.8 中的 `HashMap` 类似，都是采用“数组 + 链表 + 红黑树”的结构。
...
| ---------------- | ----------------------------------- | --------------------------------------------------- |
| 使用的锁机制     | 每个方法 `synchronized`，整张表加锁 | 局部锁（JDK7 Segment锁，JDK8 桶位锁）+ CAS 无锁优化 |
| 并发粒度         | 粗（全表）                          | 细（局部）                                          |
| 多线程访问时性能 | 差，容易阻塞                        | 高效，可并发操作不同 key                            |
| 是否适合高并发   | ❌                                   | ✅                                                   |
